...
{{selectedData.loanDetail.customerName}}
      ...
      <div class="d-grid col-12 mx-auto pt-3 pb-3">
          <button class="btn btn-primary  p-3" type="button" @click="navigateToCardDetail(selectedData.id,selectedData.loanPlan.loanSurveyEconomidTypeId,selectedData.loanDetail.loanEconomicTypeId)">
          انجام نظارت
        </button>
      </div>
    </div>
...
const navigateToCardDetail = (cardName: string, loanSurveyEconomidTypeId,loanEconomicTypeId) => {
  const newData = loanSurveyEconomidTypeId === 0 ? loanEconomicTypeId : loanSurveyEconomidTypeId;
  router.push({path: "/MonitoringRegistrationPage", query: {cardName,loanEconomicTypeId:newData}});
};
